...
public class Pickable : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            if (Type != PickableType.PINK_MUSHROOM && Type != PickableType.PINK_SEED && Type != PickableType.GOLD_SEED)
            {
                Respawn();
            }
            else
            {
                GameManager.Instance.ItemsOnGround.Remove(gameObject);
                Destroy(gameObject);
            }
        }
    }
    ...
    public void Drop(Player player = null)
    {
        ...
        if (Type == PickableType.PINK_MUSHROOM || Type == PickableType.GOLD_MUSHROOM)
        {
            ...
            {
                EnemyMushroom.mushrooms.Add(gameObject);
            }
        }
    }
